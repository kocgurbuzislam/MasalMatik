...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    // 3) Generate image via Images API (REST)
    ...
    if (!imgRes.ok) {
      ...
      // Degrade gracefully: return only story
      return res.status(200).json({ text: storyText, imageUrl: '' });
    }
    const imgJson = await imgRes.json();
    const base64 =
      (imgJson?.generatedImages?.[0]?.image?.imageBytes) ||
      (imgJson?.images?.[0]?.image?.bytes) ||
      '';
    const imageUrl = base64 ? `data:image/png;base64,${base64}` : '';
